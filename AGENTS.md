...
- **Records mode (BACKLOG-STATE phase 1).** Backlog items live as
  `self/project/tickets/<ID>.md` records (flat frontmatter over the
  ticket body); `self/project/backlog.md`'s Active section is
  generated by `node scripts/gen-backlog.mjs`. Edit records and
  regenerate — never hand-edit between the markers. Merge rule: on
  a view conflict, regenerate from the merged records. The
  validator FAILs record↔view divergence.
