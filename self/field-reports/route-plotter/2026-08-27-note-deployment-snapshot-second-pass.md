<!-- field-report: project=route-plotter · date=2026-08-27 · type=note
     · pm-skills=4.7.0
     · source=Codex synthesis of the tracked repository at public commit 6f2ac154430be665a9cb1665a6f20d1b317990e0 plus the timestamped local in-flight capture
...
Final evidence HEAD: `6f2ac154430be665a9cb1665a6f20d1b317990e0`  
Branch: `review-remediation`, equal to `origin/review-remediation` at the final source check  
Source visibility: public GitHub repository and pushed public branch

The source worktree was clean at the final evidence baseline; only this new
`field-report-route-plotter/` output became untracked afterward. At the first
orientation check, HEAD was `f1c14b96a6e69a3609f4bab4d2ad4109d099752a`
and six tracked memory/review files were modified while one continuation prompt
was untracked. A concurrent session committed and pushed that material as
`6f2ac154`; the exact earlier status, tracked diff, and untracked file are
preserved in `local/2026-08-27-export-working-tree-snapshot.md` rather than
...
| Requested surface | State at final snapshot |
| --- | --- |
...
The current v3 Git graph contains exactly one commit touching
...
The current deployment does not predate this fresh v3 history: the root commit
...
The final snapshot contains 86 commits from 2026-08-17 through 2026-08-27,
...
`<ITEM-ID>: <summary>` test, 31 of 86 subjects comply (36.05%). The 13
distinct ID families are COMPOSE, CROWD, DEMO, DOC, HEAD, MAINT, PHASE, REV,
ROUTE, SCALE, SUPPORT, UI, and UX. Conventional-commit subjects such as
`feat(ui): ...`, and non-ID headings such as `PM: ...`, were not counted.

Source: `2026-08-27-export-git-log.md`; counts reproduced from `git log` at
`6f2ac154`.
...
| `pm_skills/project/backlog.md` | 6,890 |
| `pm_skills/project/brief.md` | 3,535 |
| `pm_skills/project/conventions.md` | 2,633 |
| `pm_skills/project/decision-log.md` | 37,807 |
| `pm_skills/project/doc-deltas.md` | 1,487 |
| `pm_skills/project/file-map.md` | 36,310 |
| `pm_skills/project/tickets/REV-03.md` | 4,163 |
| `pm_skills/project/trajectory.md` | 13,222 |
| `pm_skills/project/wish-list.md` | 2,652 |
| **Total** | **263,879** |

The concatenated public memory export opens with the same source-byte
inventory and an additional post-redaction byte column.
...
6. **Release was held.** The owner directed that `review-remediation` not merge
   to `main` and that live v3.2.618 remain until an explicit release call. The
   framework does not force a merge, but this is a documented exception to the
   expected next deployment step. Source: live heading `owner sets the prune
   bar, and holds the merge`.
