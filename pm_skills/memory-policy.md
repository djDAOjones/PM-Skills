...
`prompts/end-of-task.md` (size check), `prompts/prune-memory.md`,
`prompts/doctor-memory.md`, and `prompts/roadmap-refactor.md`.
...
- `doctor-memory.md` remains the periodic deep check for structural
  drift the size check cannot see.
...
- **One writer at a time.** Project memory assumes a single active task
  ...
  `backlog.md` conflict). `prune-memory.md`'s reconcile step treats an
  unexpected concurrent edit as a stop-and-report, never a "fix".
